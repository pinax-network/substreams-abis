                ...
                &[
                    ...
                    ethabi::Token::Tuple(
                        vec![
                            ...
                            .state.0.clone().to_bytes_be() { (num_bigint::Sign::Plus,
                            bytes) => bytes, (num_bigint::Sign::NoSign, bytes) => bytes,
                            (num_bigint::Sign::Minus, _) => {
                            panic!("negative numbers are not supported") }, }
                            .as_slice(),),), ethabi::Token::Bool(self.state.1.clone()), {
                            let v = self.state.2.iter().map(| inner |
                            ...
                            ethabi::Token::Uint(ethabi::Uint::from_big_endian(match self
                            .state.3.clone().to_bytes_be() { (num_bigint::Sign::Plus,
                            bytes) => bytes, (num_bigint::Sign::NoSign, bytes) => bytes,
                            (num_bigint::Sign::Minus, _) => {
                            panic!("negative numbers are not supported") }, }
                            .as_slice(),),)
            ...
            let data = ethabi::encode(
                &[
                    ethabi::Token::Address(
                        ethabi::Address::from_slice(&self.nft_descriptor_address),
                    ),
                ],
            );
            let mut encoded = Vec::with_capacity(4 + data.len());
