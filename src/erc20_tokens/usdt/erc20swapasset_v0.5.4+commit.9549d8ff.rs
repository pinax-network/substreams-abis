...
pub mod functions {
    ...
    #[derive(Debug, Clone, PartialEq)]
    pub struct ChangeDcrmOwner {
        pub new_owner: Vec<u8>,
    }
    impl ChangeDcrmOwner {
        const METHOD_ID: [u8; 4] = [181u8, 36u8, 243u8, 165u8];
        pub fn decode(call: &substreams_ethereum::pb::eth::v2::Call) -> Result<Self, String> {
            let maybe_data = call.input.get(4..);
            if maybe_data.is_none() {
                return Err("no data to decode".to_string());
    ...
    impl substreams_ethereum::Function for ChangeDcrmOwner {
        const NAME: &'static str = "changeDCRMOwner";
        fn match_call(call: &substreams_ethereum::pb::eth::v2::Call) -> bool {
            Self::match_call(call)
        }
        fn decode(call: &substreams_ethereum::pb::eth::v2::Call) -> Result<Self, String> {
    ...
    impl substreams_ethereum::rpc::RPCDecodable<bool> for ChangeDcrmOwner {
        fn output(data: &[u8]) -> Result<bool, String> {
            Self::output(data)
        }
    }
...
pub mod events {
    ...
    #[derive(Debug, Clone, PartialEq)]
    pub struct LogChangeDcrmOwner {
        pub old_owner: Vec<u8>,
        pub new_owner: Vec<u8>,
        pub effective_height: substreams::scalar::BigInt,
    }
    impl LogChangeDcrmOwner {
        const TOPIC_ID: [u8; 32] = [
            225u8, 150u8, 141u8, 66u8, 99u8, 167u8, 51u8, 226u8, 89u8, 126u8, 246u8, 126u8, 166u8,
            173u8, 38u8, 115u8, 67u8, 187u8, 165u8, 248u8, 191u8, 15u8, 153u8, 216u8, 81u8, 144u8,
            224u8, 107u8, 5u8, 216u8, 36u8, 217u8,
        ];
        pub fn match_log(log: &substreams_ethereum::pb::eth::v2::Log) -> bool {
    ...
    impl substreams_ethereum::Event for LogChangeDcrmOwner {
        const NAME: &'static str = "LogChangeDCRMOwner";
        fn match_log(log: &substreams_ethereum::pb::eth::v2::Log) -> bool {
            Self::match_log(log)
        }
        fn decode(log: &substreams_ethereum::pb::eth::v2::Log) -> Result<Self, String> {
