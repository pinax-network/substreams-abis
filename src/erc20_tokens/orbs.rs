...
pub mod functions {
    ...
    impl Burn {
        const METHOD_ID: [u8; 4] = [8u8, 161u8, 238u8, 225u8];
        pub fn decode(call: &substreams_ethereum::pb::eth::v2::Call) -> Result<Self, String> {
            let maybe_data = call.input.get(4..);
            if maybe_data.is_none() {
                return Err("no data to decode".to_string());
        ...
        pub fn encode(&self) -> Vec<u8> {
            let data = ethabi::encode(&[ethabi::Token::FixedBytes(self.salt.as_ref().to_vec())]);
            let mut encoded = Vec::with_capacity(4 + data.len());
            encoded.extend(Self::METHOD_ID);
            encoded.extend(data);
            encoded
    ...
    impl DepositAddress {
        const METHOD_ID: [u8; 4] = [49u8, 238u8, 202u8, 244u8];
        pub fn decode(call: &substreams_ethereum::pb::eth::v2::Call) -> Result<Self, String> {
            let maybe_data = call.input.get(4..);
            if maybe_data.is_none() {
                return Err("no data to decode".to_string());
    ...
    impl substreams_ethereum::Function for DepositAddress {
    ...
    impl substreams_ethereum::rpc::RPCDecodable<Vec<u8>> for DepositAddress {
    ...
    impl Permit {
        ...
        pub fn encode(&self) -> Vec<u8> {
            let data = ethabi::encode(&[
                ethabi::Token::Address(ethabi::Address::from_slice(&self.issuer)),
                ethabi::Token::Address(ethabi::Address::from_slice(&self.spender)),
                ethabi::Token::Uint(ethabi::Uint::from_big_endian(
